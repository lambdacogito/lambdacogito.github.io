...
人类社会历史是螺旋上升的，科技史亦然。从1950s提出Artificial Intelligence概念至今，AI技术已经经历了多次春、冬季交替演进([AI Winter](https://en.wikipedia.org/wiki/AI_winter))。上一代AI技术（1970s-1980s）是基于符号（symbolic）的计算系统，以LispMacine、专家系统、日本第5代计算机系统等为代表。这类技术在知识表示、逻辑推理上有优势，但是不具备模式识别能力。此后基于统计学的AI/MachineLearning复兴（1990s-2000s），虽然彼时AI尚不普遍，但相关的数据统计分析、大数据系统、云计算基础设施，在这一时期并行快速发展。Hinton等人提出“深度学习”概念后（2006），随着AlexNet (2012) 和AlphaGo (2015) 的成功，AI技术逐步从基于浅层统计模型（例如SVM）向深层神经网络（例如CNN）演进；随着[Transformer架构](https://arxiv.org/abs/1706.03762)（2017）的发明，神经网络的复杂度呈指数级上升，其性能也推动了NLP和相关领域的技术变革。

从信息技术发展的角度来看，如今的AI浪潮也是从1990s互联网开端的信息爆炸时代的量变使然。经历过互联网发展史的人们，一定对这三次浪潮印象深刻：互联网浪潮（1995～2005）、大数据和云计算浪潮（2003～2015）、AI复兴浪潮（2012至今）。得益于最初WWW的免费和开放，让我们随着公开数据的积累（Internet）、软件规模扩大、硬件性能的提升，能够实现神经网络算法进行Scaling-law扩展，进而在2022实现AI的里程碑式创新（ChatGPT），再次燃起人类对AGI的热情和渴望。

历史视角勾勒出AI发展的“来龙”，而媒体纷扰下隐藏的细微脉络，则让我们洞察AI发展的“去脉”。以Transformer神经网络为代表的纯连接主义架构，是否是AI的普适方案？我们是否能够依此真正实现AGI？ 我们是否有其他路径和选择？虽然这些问题仍有待谨慎回答，但毋庸置疑的是，这一次以大语言模型为代表的AI技术革新，必将是人类技术发展史的关键事件之一。
...
AR和AE模型，在NLP领域取得了巨大成功。透过表象看本质，在当前的语言模型训练中，并未基于自然语言单位的语义（概念）进行学习，而是基于token组合的统计规律。换句话说，在token集合展开的组合空间里，实际的自然语言样本是空间的子集，Transformer通过神经网络参数捕获了该子集的表示规律；只要神经网络表示的参数空间足够覆盖样本序列空间，便能够成功学习子集的表示。在这个意义上，AR和AE的训练方法对于“**有限元素集合的序列空间**”具有一般性的学习能力。
...
从以上实践可见，通过将领域内问题通过适当的方法转换成序列数据后，构建离散元素集合，Transformer一般地适合**封闭/半封闭系统产生的序列化数据模式学习** [[Mirchandani2023](https://arxiv.org/pdf/2307.04721)]；这里的封闭/半封闭系统，指系统内部具有有限的状态集合、或者有限的状态生成规则（如明确的物理规律）。

## 技术热点和创新趋势

### Transformer工程优化

其背后的逻辑是大家对Scaling-law的信奉，随着训练数据规模和参数量（trilion-scale）的提升，人们期望通过工程上极致的优化，在有限资源下训练更大的模型，期待再次出现类似GPT-3的涌现时刻。
...
和单纯用好LLM相比，理解LLM的工作机制，深入探索知识表示、记忆、推理、遗忘等深层次和人类认知相关的基础技术，更有助于迈向AGI。因此AI开拓者们在Nature大声呼吁[Zador2023](https://www.nature.com/articles/s41467-023-37180-x)，我们应该注重neuroscience和AI的跨领域研究（NeuroAI），加强认知、神经科学等基础领域的教育和资金投入。
